...
const router = async (config, where, app, middleware, router) => {
  ...
  router.post('/:name', async (req, res, next) => {
    ...
    const ref = createRef(req, res),
          result = await app[name].login(ref, user, password);
  ...
  router.put('/:name', async (req, res, next) => {
    ...
    const ref = createRef(req, res),
          result = await app[name].refresh(ref, refreshToken);

    if (!result) {
      const additionalHeaders = {
        'WWW-Authenticate': 'Bearer realm="where-api", error="invalid_token", error_description="The access token is invalid or expired"',
      };

      throw new where.ServerException(status.code401.number, null, { additionalHeaders });
    }
  ...
  router.get('/:name/:scope', precheck, async (req, res, next) => {
    ...
    const result = await (!(scope in at)
      ? app[name].get(ref, scope, condition): at[scope](config, where, ref, name, condition));
  ...
  router.post('/:name/:scope', precheck, upload, async (req, res, next) => {
    ...
    if (!('post' in app[name]) || scope in at) {
      throw new where.ServerException(status.code405.number);
    }
  ...
  router.put('/:name/:scope/*cs', precheck, upload, async (req, res, next) => {
    ...
    if (!('put' in app[name]) || scope in at) {
      throw new where.ServerException(status.code405.number);
    }
  ...
  router.delete('/:name/:scope/*cs', precheck, async (req, res, next) => {
    ...
    if (!('delete' in app[name]) || scope in at) {
      throw new where.ServerException(status.code405.number);
    }

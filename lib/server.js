...
const { init, cors, upload: createUpload, ...ls } = s;

// app
import * as a from './app/index.js';
const { resolve, ...libApp } = a;

// auth
import * as libAuth from './auth/index.js';
...
const createServer = async (dir, c) => {
  ...
  const where = {
    ...
    app: {
      env, define: { status }, cq, da: { filter }, app: { resolve: resolve(dir), ...libApp }, auth: libAuth,
  ...
  // middleware
  const upload = createUpload(tmpdir, c.server.http.limit, define.filesKey);

  const middleware = Object.entries({ upload }).reduce((o, [k, v]) => {

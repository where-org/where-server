...
// common
import { common, cq, da, define,
        ConfigurationException, ServerException, ServerError } from '@where-org/where-common';
...
// middleware
import { createPrecheck, createCors, createUpload } from './middleware/index.js';
...
const mergeSpec = async (appConfig, specConfig) => {
  ...
  const resolveImportPathAppConfig = Object.fromEntries(Object.entries(appConfig).map(([k, v]) => {

    const importPath = v.app.importPath.startsWith('#module')
      ? import.meta.resolve(v.app.importPath)
      : v.app.importPath;

    return [k, { ...v, app: { ...v.app, importPath } }];
...
const createServer = async (dir, c) => {
  ...
  const env = Object.fromEntries(Object.entries({ name, version, mode, tmpdir }).map(([k, v]) => (
    [common.util.casing.camelToSnake(k).toUpperCase(), v]
  )));
  ...
  const where = {
    ...
    router: {
      define: { status }, cq, da, server: ls, log: common.init.log('router'), 
      common, ServerException, ServerError,
    },

    app: {
      define: { status }, env, cq, da: { filter }, app: { resolve: resolve(dir), ...la }, auth,
      common: { ...common, file }, ConfigurationException, ServerException, 
    },
  ...
  // cors
  server.use(createCors(c.server.cors));
  ...
  const middleware = {
    ...
    precheck: createPrecheck(c, where.router, apps),
    upload: createUpload(tmpdir, c.server.http.limit, define.filesKey),

  };

  // router
  const router = Object.fromEntries(await Promise.all(

    c.server.api.version.map(async k => {
  ...
  server.use((err, req, res, next) => {
    ...
    const { additionalHeaders = {} }  = err,
          additionalHeaderKeys = Object.keys(additionalHeaders);

    // ServerException
    if (err instanceof ServerException) {

      const { header: xWhereServerException } = ServerException;

      res.set({
        [accessControlExposeHeaders]: [accessControlExposeHeaders, xWhereServerException, ...additionalHeaderKeys].join(','),
        [xWhereServerException]: encodeURIComponent(development && err.debug ? err.debug : ''),
        ...additionalHeaders,
      });

      return res.status(err.status.code.number).send({ error: err.status.code.message });

    }

    // ServerError
    const { header: xWhereServerError } = ServerError;

    res.set({
      [accessControlExposeHeaders]: [accessControlExposeHeaders, xWhereServerError, ...additionalHeaderKeys].join(','),
      ...additionalHeaders,
    });

    // ServerError wo itoteki ni hassei saseta baai
    if (err instanceof ServerError) {
      res.set(xWhereServerError, encodeURIComponent(development && err.debug ? err.debug : ''));
      return res.status(err.status.code.number).send({ error: err.status.code.message });
    }

    // where-server-app-module ga koketeiru
    res.set(xWhereServerError, encodeURIComponent(development ? `${err.name}: ${err.message}` : ''));
